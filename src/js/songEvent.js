...
function loadItems() {
  ...
  function displayItems(items) {
    ...
    console.log(items.map(values => createHTMLString(values)).join(''));
    container.innerHTML = items.map(values => createHTMLString(values)).join('');
  }
  
  // Create HTML list item from the given data item
  function createHTMLString(values) {
    return `
      <li class="item">
        <i class="fas fa-compact-disc"></i> <div alt="${values.type}"></div>
        <span class="item__description">${values.gender}, ${values.size}</span>
      </li>
      `;
  }
